...
def load_kv_from_pinned(
    ...
    with ctx:
        ...
        src_k = buf.k[:, :n].to(k_cache.device, non_blocking=nb)
        src_v = buf.v[:, :n].to(v_cache.device, non_blocking=nb)
        k_cache.index_copy_(1, indices, src_k)
        v_cache.index_copy_(1, indices, src_v)
...
def transfer_kv(src_k, src_v, dst_k, dst_v, block_table, stream=None, buf=None) -> str:
    ...
    if _check_p2p(src_device, dst_device) and src_k is not None:
        ctx = torch.cuda.stream(stream) if stream is not None else nullcontext()
        with ctx:
            src_idx = torch.tensor(block_table, dtype=torch.long, device=src_device)
            dst_idx = src_idx.to(dst_device)
            nb = stream is not None
            dst_k.index_copy_(1, dst_idx, src_k[:, src_idx].to(dst_device, non_blocking=nb))
            dst_v.index_copy_(1, dst_idx, src_v[:, src_idx].to(dst_device, non_blocking=nb))
        return "p2p"
    else:
